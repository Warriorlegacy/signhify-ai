...
import {
  ShieldCheck, Settings, Key, Cpu, UserCircle, Eye, EyeOff,
  CheckCircle, AlertCircle, Zap, Brain, Bell, Palette,
} from "lucide-react";
    ...
    >
      <div className="flex items-center gap-3 px-5 py-4 border-b"
        ...
        <div className="w-8 h-8 rounded-lg flex items-center justify-center"
          style={{ background: `${iconColor ?? '#00e5ff'}15`, border: `1px solid ${iconColor ?? '#00e5ff'}25` }}>
          <Icon className="w-4 h-4" style={{ color: iconColor ?? '#00e5ff' }} />
        </div>
        <h2 className="text-xs font-bold uppercase tracking-widest" style={{ color: 'rgba(255,255,255,0.7)' }}>
          {title}
        </h2>
      </div>
...
export function SettingsView() {
  const { keys, saveKeys, clearKeys, preferredModel, setPreferredModel } = useSettingsStore();
  ...
  const llmProviders = LLM_PROVIDERS.filter(p => !['tavily', 'elevenlabs'].includes(p.key as string));
  const toolProviders = LLM_PROVIDERS.filter(p => ['tavily', 'elevenlabs'].includes(p.key as string));
    ...
    <div className="h-full overflow-y-auto p-6">
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        >
          ...
          <div>
            <h1 className="font-display text-xl font-bold gradient-text">Configuration</h1>
        ...
        {/* LLM Providers */}
        <SectionCard icon={Cpu} title="LLM Providers">
          <p className="text-xs mb-4" style={{ color: 'rgba(148,163,184,0.5)' }}>
            Add API keys for any provider. Signhify will use the first available key with priority: Groq → OpenAI → Anthropic → OpenRouter → Gemini.
          </p>
          {llmProviders.map(provider => (
            <ProviderRow
              ...
              onChange={v => updateKey(provider.key, v)}
            />
          ))}
        </SectionCard>
        ...
        <SectionCard icon={Zap} title="Tool Integrations" iconColor="#f59e0b">
          <p className="text-xs mb-4" style={{ color: 'rgba(148,163,184,0.5)' }}>
            Optional: add Tavily for live web search (Scout agent), ElevenLabs for voice synthesis.
          </p>
        ...
        >
          {saved ? <CheckCircle className="w-4 h-4" /> : <ShieldCheck className="w-4 h-4" />}
        ...
        {Object.values(localKeys).some(Boolean) && (
          <button
            onClick={() => { clearKeys(); setLocalKeys({}); }}
        ...
        <SectionCard icon={Brain} title="Model Routing" iconColor="#a78bfa">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              { value: "auto", label: "Auto (Recommended)", desc: "Best model per task" },
              { value: "fast", label: "Speed", desc: "Prioritize response time" },
              { value: "quality", label: "Quality", desc: "Prioritize output quality" },
              { value: "economy", label: "Economy", desc: "Minimize token usage" },
            ].map(opt => (
              <button
                key={opt.value}
                ...
                style={{
                  background: preferredModel === opt.value ? 'rgba(167,139,250,0.1)' : 'rgba(255,255,255,0.03)',
                  border: `1px solid ${preferredModel === opt.value ? 'rgba(167,139,250,0.3)' : 'rgba(255,255,255,0.06)'}`,
                }}
              >
                <p className="text-xs font-semibold mb-0.5" style={{ color: preferredModel === opt.value ? '#a78bfa' : 'rgba(255,255,255,0.7)' }}>
                  {opt.label}
                </p>
                <p className="text-[10px]" style={{ color: 'rgba(148,163,184,0.4)' }}>{opt.desc}</p>
              </button>
            ))}
          </div>
        </SectionCard>
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
              {[
                ...
                <div key={label} className="rounded-xl p-3"
                  style={{ background: 'rgba(6,6,10,0.6)', border: '1px solid rgba(30,30,46,0.6)' }}>
                  <p className="text-[10px] uppercase tracking-wider mb-1" style={{ color: 'rgba(148,163,184,0.4)' }}>
                    {label}
                  </p>
